...
module Cmp_string = struct
  type t = string
  let compare x y = if Pervasives.compare x y < 0 then LT else if Pervasives.compare x y = 0 then EQ else GT
end 
...
[*----------------------------------------------------------------------------*)
module Cmp_lex (A : Comparable)(B : Comparable) : Comparable with type t = A.t * B.t  = struct
  type t = A.t * B.t
